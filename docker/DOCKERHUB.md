...
- [`latest`, `2.4.2`](https://github.com/LanceLRQ/qwen3-asr-service/blob/main/docker/Dockerfile)
...
- [`latest-cpu`, `2.4.2-cpu`](https://github.com/LanceLRQ/qwen3-asr-service/blob/main/docker/Dockerfile.cpu) — multi-arch manifest; Docker auto-selects amd64 or arm64 by host
...
- [`latest-vllm`, `2.4.2-vllm`](https://github.com/LanceLRQ/qwen3-asr-service/blob/main/docker/Dockerfile.vllm) — vLLM-native engine (based on `vllm/vllm-openai`); adds real-time incremental partial→final streaming
...
|-----|-----------|------|-----------------|-----------|-----------|
| `latest` / `2.4.2` | `nvidia/cuda:12.1.1-runtime-ubuntu22.04` | amd64 | PyTorch (CUDA) | Required | ~4.9GB / ~8-10GB |
| `latest-cpu` / `2.4.2-cpu` | `ubuntu:22.04` | amd64 + arm64 (multi-arch) | OpenVINO (amd64: INT8 / arm64: FP32, selected at runtime) | Not required | ~2GB / ~3-4GB |
| `latest-vllm` / `2.4.2-vllm` | `vllm/vllm-openai:v0.14.0` | amd64 | vLLM (CUDA, native streaming) | Required | ~9GB / very large |
...
- [`latest-cpu`, `2.4.2-cpu`](https://github.com/LanceLRQ/qwen3-asr-service/blob/main/docker/Dockerfile.cpu) — 多架构 manifest，Docker 按本机架构自动选择 amd64 或 arm64
...
- [`latest-vllm`, `2.4.2-vllm`](https://github.com/LanceLRQ/qwen3-asr-service/blob/main/docker/Dockerfile.vllm) — vLLM 原生引擎（基底 `vllm/vllm-openai`），新增实时逐句 partial→final 增量流式
...
|-----|---------|------|---------|-----------|---------|
| `latest` / `2.4.2` | `nvidia/cuda:12.1.1-runtime-ubuntu22.04` | amd64 | PyTorch (CUDA) | 需要 | ~4.9GB / ~8-10GB |
| `latest-cpu` / `2.4.2-cpu` | `ubuntu:22.04` | amd64 + arm64（多架构） | OpenVINO（amd64: INT8 / arm64: FP32，运行时自选） | 不需要 | ~2GB / ~3-4GB |
| `latest-vllm` / `2.4.2-vllm` | `vllm/vllm-openai:v0.14.0` | amd64 | vLLM（CUDA，原生流式） | 需要 | ~9GB / 体积较大 |
